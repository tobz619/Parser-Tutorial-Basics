...
instance Functor Parser where
    fmap f (Parser xs) = Parser $ \inp -> map (\(v,inp) -> (f v, inp)) (xs inp)
...
-- both argument parsers p and q are appled to the same input string and the result is concatenated to form 
...
ogString "" = result ""
ogString (x:xs) = char x >> (
                   ogString xs >>
                   result (x:xs))

--original many
ogMany :: Parser a -> Parser [a]
ogMany p = neParse `plus` result []
